...
# =============================================================================
# NIGHTLY-base variant of vllm_disagg_inference.glmv5.1_v0.24.local — identical
# build (v0.24-rebased GLM-DSA WideEP vLLM from the LOCAL `vllmsrc` context +
# vllm-router + rocm.py GCN-arch fix) but layered on a specific ROCm NIGHTLY base
# image instead of the v0.24.0 release image.
#
#   >>> ONLY functional change vs the .local variant: the BASE_IMAGE (see below) <<<
#   Base : vllm/vllm-openai-rocm:nightly-94c0ef300180f8fd1071d9cbe7270a8348155f94
#   vLLM : still the v0.24 rebase (glm5.1-dsa-wideEP_on_v0.24.0) via --build-context
#
# Build (context = MAD repo root; vLLM source passed as a named context):
#   DOCKER_BUILDKIT=1 docker build \
#     -f docker/vllm_disagg_inference.glmv5.1_v0.24.nightly.ubuntu.amd.Dockerfile \
...
#     -t vllm-disagg:glmv5.1-v0.24-nightly .
#
# Fast-first-build defaults: reuse the nightly image's bundled AITER/MoRI and skip
# NIXL. Flip REBUILD_AITER/MORI=1 and WITH_NIXL=1 later if NIAH needs the validated
# aiter e03fa6040 / mori pins.
# =============================================================================
# >>> THE CHANGE: base image is the specified ROCm nightly (was v0.24.0 release) <<<
ARG BASE_IMAGE=vllm/vllm-openai-rocm:nightly-94c0ef300180f8fd1071d9cbe7270a8348155f94
FROM ${BASE_IMAGE}
...
ARG REBUILD_AITER=0
ARG REBUILD_MORI=0
RUN mkdir -p /app && echo "BASE_IMAGE=${BASE_IMAGE} (v0.24 rebase on ROCm nightly)" >> /app/versions.txt
...
# 3.0.2-0ubuntu1.23). NOTE: this must match the NIGHTLY base's installed libssl3; if
# the nightly ships a different libssl3, re-fetch the matching libssl-dev .deb (see
# docker/debs/) or set WITH_ROUTER=0. pkg-config/gcc/git/curl come from the base.
ARG WITH_ROUTER=1
...
#     Path is globbed (python3.*) because the nightly base may use a different
#     Python minor version than the release image.
# -----------------------------------------------------------------------------
RUN ROCM_PY="$(ls /usr/local/lib/python3*/dist-packages/vllm/platforms/rocm.py /usr/lib/python3*/dist-packages/vllm/platforms/rocm.py 2>/dev/null | head -1)" && \
    test -n "$ROCM_PY" && test -f "$ROCM_PY" && \
    ...
    echo "PATCH OK: rocm.py GCN-arch circular-import + torch.cuda fallback fixed ($ROCM_PY)" && \
