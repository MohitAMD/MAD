#!/bin/bash
# =============================================================================
# 1P/1D disaggregated (2 nodes) NIAH repro for GLM-5.1-FP8 on the v0.24-rebased
# GLM image built on the ROCm NIGHTLY base (vllm-disagg:glmv5.1-v0.24-nightly),
# through the REAL vllm-router.
#
# FAITHFUL to vllm issue #47042: thinking ON (NIAH_THINKING=1) + max_tokens 2048 +
# sizes 2000/8000/20000/35000, so the long-context DECODE path (where the sparse-MLA
# collapse shows up) is actually exercised.
#
# Submit (exclude known-bad node 119):
#   sbatch -p amd-rccl -N 2 --gres=gpu:8 --time=12:00:00 --requeue \
#          --exclude=useocpm2m-097-119 \
#          --job-name=glm-1p1d-niah-nightly --export=ALL \
#          glm5.1_notes/sbatch_1p1d_niah_nightly.sh 2>&1 \
#     | tee output_sbatch_1p1d_niah_nightly.txt
# =============================================================================
#SBATCH --job-name=glm_1p1d_niah_nightly
...
export NIAH_TOPO="MI300X 1P1D EP8 (disagg, MoRIIO)"
...
echo "=== launching 1p1d NIAH nightly (job $SLURM_JOB_ID) ==="
bash glm5.1_notes/run_one_config.sh 1 1 niah_nightly_1p1d niah
