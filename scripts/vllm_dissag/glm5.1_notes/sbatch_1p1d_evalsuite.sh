...
# =============================================================================
#SBATCH --job-name=glm_1p1d_evalsuite
...
# ---- 4+5. run NIAH smoke + eval suite in a throwaway container (--network host)
# Same image => has python3 + openai; host network => 127.0.0.1:30000 reaches router.
echo "=== running evals in container (job $SLURM_JOB_ID) ==="
docker run --rm --network host \
  ...
  --entrypoint bash "$IMAGE" -c '
    ...
    if [ "${RUN_SMOKE:-0}" = "1" ]; then
      echo "----- NIAH smoke probe (/v1/completions) -----"
      NIAH_URL="http://127.0.0.1:30000/v1/completions" \
      NIAH_MODEL="$MODEL_PATH" \
      NIAH_ENDPOINT=completions \
      NIAH_COMBOS="2000/2048,8000/2048,20000/2048,96000/32000" \
      NIAH_SEEDS="0,1,2" \
      NIAH_TIMEOUT=5400 NIAH_WARMUP=1 \
      NIAH_TOPO="MI300X 1P1D EP8 (disagg, /v1/completions) [WideEP]" \
        python3 "$REPO/benchmark_niah.py" 2>&1 | tee "$LOGDIR/niah_smoke_1p1d.log"
    else
      echo "----- NIAH smoke probe SKIPPED (RUN_SMOKE=0; already collected) -----"
    fi

    echo "----- Cohere Accuracy Eval Suite (profile=pre_release, GLM only) -----"
    cd "$EVAL_SUITE"
    python3 runners/run_suite.py \
      ...
      --endpoint "http://127.0.0.1:30000/v1" \
      --no-stop-on-fail \
      --output-dir "$LOGDIR/eval_suite_out" 2>&1 | tee "$LOGDIR/eval_suite_1p1d.log"

    # durable copies
    cp -f "$EVAL_SUITE/outputs/latest/results.json" "$LOGDIR/eval_suite_out/results.json" 2>/dev/null || true
    echo "eval outputs -> $LOGDIR/eval_suite_out"
  '
