...
# FROM-SCRATCH image (glm5.1-fp8-disagg:mi300x-fromscratch-aiter017) and run the FULL
...
#     glm5.1_notes/sbatch_2p2d_evalsuite_fromscratch_a017.sh
# =============================================================================
#SBATCH --job-name=glm_2p2d_es_fromscratch_a017
...
bash glm5.1_notes/run_one_config.sh 2 2 evalsuite_2p2d_fromscratch_a017 keepalive &
KA_PID=$!
...
teardown() {
  ...
  srun --overlap --ntasks-per-node=1 bash -c '
    for n in glm_evalsuite_2p2d_fromscratch_a017 container_GLM-5.1-FP8; do
...
docker run --rm --name glm_evalsuite_2p2d_fromscratch_a017 --network host \
