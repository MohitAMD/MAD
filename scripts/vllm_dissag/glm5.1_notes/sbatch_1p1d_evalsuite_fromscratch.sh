...
# FROM-SCRATCH image (glm5.1-fp8-disagg:mi300x-fromscratch, built entirely from
...
#     glm5.1_notes/sbatch_1p1d_evalsuite_fromscratch.sh
# =============================================================================
#SBATCH --job-name=glm_1p1d_es_fromscratch
...
bash glm5.1_notes/run_one_config.sh 1 1 evalsuite_1p1d_fromscratch keepalive &
KA_PID=$!
...
teardown() {
  ...
  srun --overlap --ntasks-per-node=1 bash -c '
    for n in glm_evalsuite_1p1d_fromscratch container_GLM-5.1-FP8; do
...
docker run --rm --name glm_evalsuite_1p1d_fromscratch --network host \
