#!/bin/bash
# =============================================================================
# 1P1D disagg NIAH for GLM-5.1-FP8 on the PR#47766-patched image
# (vllm-disagg:glmv5.1-v0.24-pr47766 = stock v0.24.0 + complete 6-field #47766
# metadata-key patch + vllm-router PR#181 + rocm.py boot fix), via raw /v1/completions.
#
# KEY vs the rebase-image wrappers: this image does NOT bake the DSA/MoRIIO fixes in
# source, so it MUST run the runtime patchers -> GLM_SKIP_PATCHERS=0 (the launcher
# applies apply_glm_dsa_*.py at boot). NIAH_TIMEOUT bumped to 5400 so 96k/32k doesn't
# time out (that was the only NO-RESULT in 202983).
#
# Submit:
#   sbatch -p amd-rccl -N 2 --gres=gpu:8 --time=12:00:00 --requeue \
#          --exclude=useocpm2m-097-119 \
#          --job-name=glm-1p1d-niah-pr47766-cmpl --export=ALL \
#          glm5.1_notes/sbatch_1p1d_niah_pr47766_completions.sh 2>&1 \
#     | tee output_sbatch_1p1d_niah_pr47766_completions.txt
# =============================================================================
#SBATCH --job-name=glm_1p1d_niah_pr47766_cmpl
...
# --- PR#47766-patched image (stock v0.24.0 + 6-field key + router PR#181) ------
export DOCKER_IMAGE_NAME=vllm-disagg:glmv5.1-v0.24-pr47766
export IMAGE="$DOCKER_IMAGE_NAME"
export WANT_IMAGE_ID="${WANT_IMAGE_ID:-5f0f3679b47e}"
export IMAGE_TAR=/shared_inference/mdeopuja/model_blog_logs/docker_images/glmv5.1-v0.24-pr47766.tar
# CRITICAL: fixes are NOT baked in this image -> run the runtime DSA/MoRIIO patchers.
export GLM_SKIP_PATCHERS=0
export SKIP_RUNTIME_PATCH=0
export PROXY_TYPE=vllm_router
export VLLM_GCN_ARCH=gfx942

# --- NIAH grid (raw /v1/completions, no reasoning parser) + 96k/32k ------------
...
export NIAH_TIMEOUT=5400          # 96k-in/32k-out is slow; avoid the 202983 timeout
export NIAH_TOPO="MI300X 1P1D EP8 (disagg, /v1/completions) [v0.24.0 + PR#47766 6-field + patchers]"
...
# --- launch 1P1D NIAH (/v1/completions) through the router, patchers ON --------
echo "=== launching 1p1d NIAH pr47766 /v1/completions (job $SLURM_JOB_ID) ==="
bash glm5.1_notes/run_one_config.sh 1 1 niah_pr47766_1p1d_cmpl niah
