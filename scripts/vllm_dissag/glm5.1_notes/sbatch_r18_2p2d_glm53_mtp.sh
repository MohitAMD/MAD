...
# GLM-5.3-FP8 WideEP (EP8) 2P2D disagg — MTP (speculative decoding) ENABLED
#
# Goal: port Recipe 14's single-node MTP onto the Recipe 17 WideEP 2P2D stack.
...
#   sbatch -p amd-rccl -N 4 --gres=gpu:8 --time=8:00:00 --requeue --switches=1 \
#     --exclude=useocpm2m-097-[119,135,142] --job-name=r18_2p2d_glm53_mtp \
#     glm5.1_notes/sbatch_r18_2p2d_glm53_mtp.sh
# =============================================================================
#SBATCH --job-name=r18_2p2d_glm53_mtp
...
echo "=== r17+mtp 2P2D: pre-launch cleanup (job ${SLURM_JOB_ID:-local}) ==="
srun --overlap --ntasks-per-node=1 bash -c '
...
run_perf() {
  ...
  echo "=== r17+mtp 2P2D PERF: combos='$combos' con='$con' tag='$tag' MTP=${MTP} (job ${SLURM_JOB_ID:-local}) ==="
  export BENCHMARK_COMBINATIONS="$combos"
  export BENCHMARK_CON="$con"
  bash glm5.1_notes/run_one_config.sh 2 2 "$tag" long_context
}
...
run_perf "8000/1000"  "1 4 8 16 32"  r18_2p2d_glm53_mtp_8k1k
run_perf "8000/8000"  "1 4 8"        r18_2p2d_glm53_mtp_8k8k

# ---- post-run validation -----------------------------------------------------
JOB_LOG_DIR="/shared_inference/mdeopuja/model_blog_logs/${SLURM_JOB_ID:-local}"
DECODE_LOG="${JOB_LOG_DIR}/decode_NODE2.log"
echo ""
echo "############################################################################"
echo "# DONE  Recipe 17 + MTP (v0.27.1 + aiter019 + PR#176 overlay)  2P2D  MTP=${MTP} n=${NSPEC}"
