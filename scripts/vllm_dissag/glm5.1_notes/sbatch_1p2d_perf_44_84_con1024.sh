#!/bin/bash
# =============================================================================
# 1P2D (EP16) disagg PERF -- 4000/4000 AND 8000/4000 @ concurrency 1024 (probe
# for a higher peak). The full sweep (job 209754) only reached con=512 on these
# two shapes, yet 1P2D sustained con=1024 at 8k/1k (18286 tok/s), so con=1024
# here MAY climb above the con=512 points (9416 / 12725). Heavier 4k-token output
# could instead tail-deadlock the single-prefill KV path -- this run settles it.
# FROM-SCRATCH image (af5d3ad886cc). 3 nodes = 1 prefill + 2 decode.
#
# Submit (leaf-pinned, 3 nodes; exclude faulted nodes):
#   sbatch --exclude=<faulted> --job-name=glm-1p2d-44-84-con1024 --export=ALL \
#     glm5.1_notes/sbatch_1p2d_perf_44_84_con1024.sh
# =============================================================================
#SBATCH --job-name=glm_1p2d_44_84_con1024
...
echo "=== 1P2D perf: 4k/4k + 8k/4k @ con 1024 (defer=${MORIIO_DEFER_TIMEOUT}) (job $SLURM_JOB_ID) ==="
export BENCHMARK_COMBINATIONS="4000/4000 8000/4000"
export BENCHMARK_CON="${BENCHMARK_CON:-1024}"
bash glm5.1_notes/run_one_config.sh 1 2 perf_1p2d_44_84_con1024 long_context

echo "=== DONE 1P2D perf 4k/4k+8k/4k con=${BENCHMARK_CON} (job $SLURM_JOB_ID); results in /shared_inference/mdeopuja/model_blog_logs/${SLURM_JOB_ID}/ ==="
