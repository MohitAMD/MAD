...
# GLM-5.3-FP8 WideEP (EP8) 1P1D disagg — MTP (speculative decoding) ENABLED
#
# Goal: port Recipe 14's single-node MTP onto the Recipe 17 WideEP 1P1D stack.
...
#   sbatch -p amd-rccl -N 2 --gres=gpu:8 --time=8:00:00 --requeue --switches=1 \
#     --exclude=useocpm2m-097-[119,135,142] --job-name=r18_1p1d_glm53_base \
#     glm5.1_notes/sbatch_r18_1p1d_glm53_base.sh
# =============================================================================
#SBATCH --job-name=r18_1p1d_glm53_base
...
echo "=== r17+mtp 1P1D: pre-launch cleanup (job ${SLURM_JOB_ID:-local}) ==="
srun --overlap --ntasks-per-node=1 bash -c '
...
run_perf() {
  ...
  echo "=== r17+mtp 1P1D PERF: combos='$combos' con='$con' tag='$tag' MTP=${MTP} (job ${SLURM_JOB_ID:-local}) ==="
  export BENCHMARK_COMBINATIONS="$combos"
  export BENCHMARK_CON="$con"
  bash glm5.1_notes/run_one_config.sh 1 1 "$tag" long_context
}
...
run_perf "8000/1000"  "1 4 8 16 32"  r18_1p1d_glm53_base_8k1k
run_perf "8000/8000"  "1 4 8"        r18_1p1d_glm53_base_8k8k
...
echo ""
echo "############################################################################"
echo "# DONE  Recipe 17 + MTP (v0.27.1 + aiter019 + PR#176 overlay)  1P1D  MTP=${MTP} n=${NSPEC}"
