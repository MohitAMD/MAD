#!/bin/bash
# =============================================================================
# 2P2D (EP16) disagg PERF sweep for GLM-5.1-FP8 on the FROM-SCRATCH image
...
#
# LEAF-PINNED: submit with -w of 4 nodes in ONE leaf (RDMA locality). Example:
#   sbatch -p amd-rccl -N 4 --gres=gpu:8 --time=24:00:00 --requeue \
#     -w <leafNodeA>,<leafNodeB>,<leafNodeC>,<leafNodeD> \
#     --job-name=glm-2p2d-perf-fromscratch --export=ALL \
#     glm5.1_notes/sbatch_2p2d_perf_fromscratch.sh
# =============================================================================
#SBATCH --job-name=glm_2p2d_perf_fromscratch
...
echo "=== 2P2D perf pass 1: 8k/1k,4k/4k,8k/4k @ 8/32/64/128/256 (job $SLURM_JOB_ID) ==="
export BENCHMARK_COMBINATIONS="8000/1000 4000/4000 8000/4000"
export BENCHMARK_CON="8 32 64 128 256"
bash glm5.1_notes/run_one_config.sh 2 2 perf_main_2p2d_fromscratch long_context

# ---- pass 2: 96k/32k @ con 8/32/64/128 -------------------------------------
echo "=== 2P2D perf pass 2: 96k/32k @ 8/32/64/128 (job $SLURM_JOB_ID) ==="
export BENCHMARK_COMBINATIONS="96000/32000"
export BENCHMARK_CON="8 32 64 128"
bash glm5.1_notes/run_one_config.sh 2 2 perf_96k_2p2d_fromscratch long_context

echo "=== DONE 2P2D perf sweep (job $SLURM_JOB_ID); results in /shared_inference/mdeopuja/model_blog_logs/${SLURM_JOB_ID}/ ==="
