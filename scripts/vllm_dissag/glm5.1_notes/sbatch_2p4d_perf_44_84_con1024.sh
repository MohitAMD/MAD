#!/bin/bash
# =============================================================================
# 2P4D (EP32) disagg PERF -- 4000/4000 AND 8000/4000 @ concurrency 1024 (probe
# for a higher peak). The peak sweep (job 208491) reached only con=512 on these
# shapes, but 2P4D sustained con=1024 at 8k/1k (19175 tok/s), so con=1024 here
# MAY climb above the con=512 points (6835 / 9667). Heavier 4k-token output could
# instead saturate the KV path -- this run settles it. FROM-SCRATCH image
# (af5d3ad886cc). 6 nodes = 2 prefill + 4 decode.
#
# Submit (leaf-pinned, 6 nodes; exclude faulted nodes):
#   sbatch --exclude=<faulted> --job-name=glm-2p4d-44-84-con1024 --export=ALL \
#     glm5.1_notes/sbatch_2p4d_perf_44_84_con1024.sh
# =============================================================================
#SBATCH --job-name=glm_2p4d_44_84_con1024
...
echo "=== 2P4D perf: 4k/4k + 8k/4k @ con 1024 (defer=${MORIIO_DEFER_TIMEOUT}) (job $SLURM_JOB_ID) ==="
export BENCHMARK_COMBINATIONS="4000/4000 8000/4000"
export BENCHMARK_CON="${BENCHMARK_CON:-1024}"
bash glm5.1_notes/run_one_config.sh 2 4 perf_2p4d_44_84_con1024 long_context

echo "=== DONE 2P4D perf 4k/4k+8k/4k con=${BENCHMARK_CON} (job $SLURM_JOB_ID); results in /shared_inference/mdeopuja/model_blog_logs/${SLURM_JOB_ID}/ ==="
